...
class NetworkClient:
    def __init__(self, rdm):
        ...
        self.ready = True
        self.rdm.lg('ROBOT', 0, 'Подключаюсь к сокету: ' + self.config['host'] + ":" + self.config['port'])
        try:
            self.rx_socket.connect((self.config['host'], int(self.config['port'])))
            self.rdm.lg('ROBOT', 0, 'Подключён к сокету: '+self.config['host']+":"+self.config['port'])
